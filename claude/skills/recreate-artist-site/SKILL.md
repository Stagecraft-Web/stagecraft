...
- **Keystatic round-trip** — `npm run dev` and walk every singleton (Site Settings, Header & Navigation, Appearance) and every collection item (each page, each release, each tour date, each press quote, etc.) in the Keystatic admin: open the entry and click **Save**. Even with no field changes, save normalises Keystatic's serialisation — discriminated-union conditionals, optional-image shapes, default-value coercion. After saving everything, run `npm run build` and confirm there are zero new errors. This catches the class of bugs where the recreation script writes a YAML/JSON shape that parses on read but Keystatic re-serialises differently on save (an invisible authoring tax — every editor's first save would otherwise blow up the build). Automate with Playwright if the site has many entries; loop over the admin URLs from the nav, click `[data-testid="save-button"]` (or equivalent), wait for the success toast.
