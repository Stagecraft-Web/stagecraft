...
**Dev fallback (platform not configured):** writes JSON directly to `src/content/pages/<slug>.json`. Detected by missing `STAGECRAFT_PLATFORM_URL`, `SITE_ID`, or `STAGECRAFT_BROKER_SECRET`.
...
| `SITE_ID` | prod | Site row id on the platform. |
