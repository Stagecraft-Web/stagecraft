 ...
 * `{% responsive-embed %}` instead — it shares the same parser and adds an
...
export const keystatic: KeystaticContentComponent = block({
  label: "Embed",
  description:
    "Embed a player at its native size. Paste the raw HTML from the service's 'Share / Embed' UI (Spotify, Bandcamp, SoundCloud, Apple Music, etc.). For embeds that should scale to fill the column, use Responsive Embed instead.",
  schema: {
    code: fields.text({
      label: "Embed code",
      description:
